function connectWebSocket() {
  const ws = new WebSocket('wss://vault-4n5e.onrender.com');
  ...
  ws.onerror = (error) => {
    ...
    alert('WebSocket connection failed. Attempting to reconnect...');
  };
  ...
  function updateExpiringList() {
    ...
    if (expiringProducts.length === 0) {
      expiringList.innerHTML = '<p>No products expiring soon.</p>';
    } else {
  ...
  function deleteProduct(productId) {
    if (confirm('Are you sure you want to delete this product?')) {
      if (ws.readyState === WebSocket.OPEN) {
        ...
        alert('WebSocket not connected. Cannot delete product.');
      }
    }
  }
  ...
  document.getElementById('product-form').addEventListener('submit', (e) => {
    ...
    if (ws.readyState === WebSocket.OPEN) {
      ...
      console.error('WebSocket not connected');
      alert('WebSocket not connected. Cannot submit product.');
    }
